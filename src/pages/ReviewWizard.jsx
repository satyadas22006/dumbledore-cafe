...
export default function ReviewWizard({ onComplete, onNavigate, theme, twin, setTwin }) {
  ...
  // FIX: The <video> element only mounts once `cameraStream` is truthy
  // (it's conditionally rendered below), so `videoRef.current` was still
  // null at the exact moment `startCamera` tried to assign `srcObject`
  // right after `getUserMedia` resolved — the stream was captured, but
  // never actually piped into the (not-yet-existing) <video> tag, which
  // is why the preview stayed black and every capture came out empty.
  // This effect re-runs after the video element has mounted and safely
  // attaches the live stream to it every time the stream changes.
  useEffect(() => {
  ...
  // Detect whether the device actually has more than one camera before
  // showing a flip control (avoids showing a useless button on laptops).
  useEffect(() => {
  ...
  const startCamera = async (mode = facingMode) => {
    ...
    try {
      ...
      // NOTE: srcObject assignment now happens in the useEffect above,
      // once the <video> element guaranteed to exist in the DOM.
    } catch (err) {
  ...
  // Flip between front ("user") and back ("environment") camera, phone-style.
  const flipCamera = () => {
  ...
  const capturePhoto = () => {
    if (videoRef.current && canvasRef.current) {
      ...
      // Only mirror the snapshot for the front-facing camera, matching
      // the mirrored live preview. The back camera should capture normally.
      if (facingMode === 'user') {
                    ...
                    <div className="w-full rounded-2xl p-3 bg-gradient-to-br from-[#DFD4B7] to-[#B09F7A] border-2 border-[#534538] flex flex-col md:flex-row gap-3 items-center">
                      <div className="w-full md:w-[60%] aspect-[4/3] rounded-xl p-1.5 bg-[#F3ECDB] border border-[#534538] flex flex-col">
                        <div className="w-full flex-1 bg-black rounded overflow-hidden relative flex items-center justify-center">
                          {/* FIX: video element is now always mounted (instead of only
                              when cameraStream is truthy) so `videoRef.current` is never
                              null when we try to attach the stream. Visibility is toggled
                              instead, which is what actually makes the live preview show up. */}
                          <video
                            ref={videoRef}
                            autoPlay
                            playsInline
                            ...
                            style={{
                              display: cameraStream && !capturedImage ? 'block' : 'none',
                          ...
                          {/* Flip camera control — phone-style, only shown once the
                              device is confirmed to have more than one camera and a
                              live (non-captured) preview is showing. */}
                          {cameraStream && !capturedImage && hasMultipleCameras && (
                            <button
                              onClick={flipCamera}
                              title="Flip camera"
                              className="absolute bottom-1.5 right-1.5 w-7 h-7 rounded-full bg-black/50 border border-white/40 text-white flex items-center justify-center hover:bg-black/70 transition-colors"
                            >
                              <RotateCcw size={13} />
                            </button>
                          )}
