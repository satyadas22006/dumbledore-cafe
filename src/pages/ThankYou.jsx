...
const CuteEnvelope = ({ className }) => (
  <motion.div 
    ...
    className={`absolute z-40 w-20 h-14 bg-[#EBE3D5] shadow-sm border border-[#C2B4A3] flex items-center justify-center cursor-pointer ${className}`}
  >
    {/* Envelope Flaps */}
    <div className="absolute top-0 inset-x-0 h-0 border-t-[28px] border-x-[40px] border-t-[#D5C8B5] border-x-transparent drop-shadow-sm z-10" />
    <div className="absolute bottom-0 inset-x-0 h-0 border-b-[28px] border-x-[40px] border-b-[#EBE3D5] border-x-transparent z-20" />
    
    <div className="absolute z-30 text-[#D93838] text-xl transform -translate-y-1">❤️</div>
...
export default function ThankYou({ reviewData, onNavigate }) {
  // Generate a random 6 digit check number
  const checkNumber = useMemo(() => Math.floor(Math.random() * 900000) + 100000, []);
  
  // Safe fallbacks if coming directly to this page
  const data = reviewData || {
    name: "Cozy Wanderer",
    items: ["Signature Brew", "Warm Pastry"],
    vibe: "✨ Main Character Energy"
  };

  const handleShare = async (platform) => {
    const memoryText = data.text || data.review || "Such a cozy spot!";
   ...
   if (platform === 'twitter') window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}`, '_blank', 'noopener,noreferrer');
      ...
      alert("Screenshot your Guest Check and tag us on your IG Story! ✨📸");
    }
  };

  // Grid background from the reference images
  ...
  return (
    <div style={gridBackgroundStyle} className="min-h-screen relative overflow-hidden flex flex-col items-center justify-center p-4 select-none font-sans">
      ...
      {/* 1. Deep Red Background Mat Accent (From image_119315) */}
      ...
      <FloatingDoodle char="⭐" className="top-[10%] right-[25%] text-[#D93838] !opacity-80 drop-shadow-sm" />
      <FloatingDoodle char="⭐" className="bottom-[20%] right-[15%] text-[#D93838] !opacity-80 drop-shadow-sm" delay={1} />
      <FloatingDoodle char="🥐" className="top-[20%] left-[15%] !opacity-90 drop-shadow-md text-6xl" delay={2} />

      {/* --- CENTRAL CRUMPLED KRAFT PAPER --- */}
      <motion.div 
      ...
      >
        ...
        <div 
          ...
          style={{ 
            filter: 'url(#crumple)',
            clipPath: 'polygon(2% 1%, 98% 3%, 99% 97%, 3% 99%, 0% 50%)' // Slightly irregular edges
          }}
        />

        {/* Cute Envelope Pinned to the side */}
        ...
        >
          ...
          <div className="w-full bg-[#f3e1cb] relative text-[#173e87] flex flex-col font-sans pb-4 overflow-hidden border border-black/10 shadow-[2px_4px_15px_rgba(0,0,0,0.15)]">
            ...
            <div className="bg-[#fbd8c9] p-4 flex flex-col pt-6 relative">
              ...
              <div className="grid grid-cols-4 border-b-2 border-[#173e87] text-[7px] font-bold text-center tracking-tighter uppercase">
                <div className="border-r-2 border-[#173e87] py-0.5">Table</div>
                <div className="border-r-2 border-[#173e87] py-0.5">Persons</div>
                <div className="border-r-2 border-[#173e87] py-0.5">Waiter</div>
              ...
              <div className="grid grid-cols-4 h-6 border-b-2 border-[#173e87]">
                ...
                <div className="border-r-2 border-[#173e87]"></div>
                <div className="border-r-2 border-[#173e87]"></div>
                <div className="flex items-center justify-center font-mono text-sm text-[#df3131] opacity-80 tracking-widest" style={{ transform: 'scaleY(1.2)' }}>
                  {checkNumber}
                </div>
              </div>
            </div>
            ...
            <div className="relative flex-1 w-full flex flex-col pt-1 bg-[#EFE0CB]">
              ...
              <div className="flex flex-col z-10 relative">
                ...
                <div className="h-[40px] border-b-[1.5px] border-[#173e87] flex items-end pb-1 relative px-3 overflow-visible">
                  <span className="font-cursive text-4xl text-[#df3131] font-black absolute bottom-1 left-20 z-20" style={{ rotate: '-12deg', mixBlendMode: 'multiply' }}>
                    :)
                  </span>
                </div>
                <div className="h-[40px] border-b-[1.5px] border-[#173e87]"></div>
              </div>
            </div>
            ...
            <div className="px-3 pt-2 relative bg-[#EFE0CB]">
              ...
              <div className="absolute top-1 right-8 z-20">
                <span 
                  className="font-cursive text-2xl text-[#df3131] font-black transform origin-bottom-right inline-block whitespace-nowrap"
                  style={{ rotate: '-6deg', mixBlendMode: 'multiply' }}
                >
                  - {data.name}
                </span>
              </div>
      ...
      <div className="absolute bottom-6 right-6 md:bottom-10 md:right-10 flex flex-col items-end gap-4 z-50">
        ...
        <button 
          ...
          className="bg-[#FFFDF9] border-[3px] border-[#7A2021] text-[#7A2021] font-serif font-black text-sm px-6 py-3 shadow-[4px_4px_0_#7A2021] hover:translate-y-[2px] hover:shadow-[2px_2px_0_#7A2021] transition-all transform -rotate-2"
        >
          View The Chronicles →
        </button>
      </div>
